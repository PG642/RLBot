...
from striker_training import StrikerEasy

def make_match_config():
    match_config = read_match_config_from_file(Path('striker_match.cfg'))
...
def make_default_playlist(checkpoints, repetitions, shot_ids) -> Playlist:
    ...
    for checkpoint in checkpoints:
        for repetition in range(repetitions):
            for id in shot_ids:
                ex = StrikerEasy("shot: " + str(id), path=checkpoint, repetition=repetition, shot=id)
